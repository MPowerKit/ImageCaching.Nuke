...
xcodebuild archive -destination "generic/platform=macOS,variant=Mac Catalyst" -project NukeProxy.xcodeproj -scheme NukeProxy -configuration Release -archivePath Output/Output-maccatalyst SKIP_INSTALL=NO ENABLE_BITCODE=NO BUILD_LIBRARY_FOR_DISTRIBUTION=YES
...
echo "sharpie bind"
sharpie bind --sdk=iphoneos18.2 --output="Output" --namespace="MPowerKit.NukeProxy" --scope="Output/NukeProxy.xcframework/ios-arm64/NukeProxy-ios.framework/Headers" "Output/NukeProxy-ios.xcframework/ios-arm64/NukeProxy.framework/Headers/NukeProxy-Swift.h"
